...
class Downloader:
    ...
    async def download_stream(self, streamer: str):
        ...
        if return_code == 0:
            logger.success(
                f"Descarga de VOD para {streamer} completada: {local_filepath}"
            )
            ...
            logger.info(
                "🚀 Iniciando pipeline de procesamiento para el VOD recién descargado..."
            )
            # dry_run=False porque esto es una ejecución de producción
            await process_single_file(
                self.config, str(local_filepath), streamer, dry_run=False
            )

        else:
            logger.error(
                f"Streamlink para {streamer} finalizó con código de error: {return_code}"
            )
